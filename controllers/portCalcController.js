    ...
    port.then((portfRes)=>{
        ...
        portfRes.forEach(element => {
            let p = new Promise((resolve, reject)=>{
                resolve(getDataRange(element.stock_symbol, element.purchase_date ,  '2023-09-16')) // TODO UPDATE THIS
            })
            proms.push(p);
...
const spend = (req, res) =>{
    ...
    port.then((portfRes)=>{
        ...
        let p = new Promise((resolve, reject)=>{
            resolve(getDataRange(port[0].stock_symbol, port[0].purchase_date ,  '2023-09-16')) // TODO UPDATE THIS
        }).then((resp)=>{
            // logger.info(p);
